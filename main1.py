...
# Настройки захвата
AUTO_CLAIM_THRESHOLD = 90  # Юзернеймы с рейтингом 90+ забираются автоматом
BASE_CHECK_DELAY = 1.3     # Пауза между проверками (защита от бана)
VOWELS = "aeiouy"
...
class UsernameManager:
    ...
    async def claim_via_group(self, username: str, user_id: int):
        try:
            ...
            await self.bot.send_message(user_id, f"🔥 **НИК ЗАБРОНИРОВАН!**\n@{username} теперь привязан к твоему новому чату!")
            return True
        except Exception as e:
            if "CHANNELS_ADMIN_PUBLIC_SEGMENTS_TOO_MANY" in str(e):
                await self.bot.send_message(user_id, "❌ Лимит публичных чатов (10) исчерпан! Удали лишние.")
            else:
            ...
            for username in list(self.monitored[user_id]):
                if await self.manager.check_username_available(username):
                    ...
                    if score >= AUTO_CLAIM_THRESHOLD:
                        ...
                        await self.bot.send_message(user_id, f"🔔 @{username} освободился!")
...
async def main():
    ...
    dp = Dispatcher()
    mgr = UsernameManager(client, bot)
    sniper = SnipeManager(bot, mgr)
    ...
    async def cmd_start(m: types.Message):
        ...
        await m.answer("👋 Бот-снайпер готов к работе!\nЕсли найду ник 90+, заберу в чат автоматически.", reply_markup=kb)
    ...
    async def gen_read(call: types.CallbackQuery):
        ...
        for n in mgr.generate_readable(8):
            if await mgr.check_username_available(n):
                ...
                if score >= AUTO_CLAIM_THRESHOLD: 
                    await mgr.claim_via_group(n, call.from_user.id)
                else: 
                    await call.message.answer(f"✅ Свободен: @{n} (Оценка: {score})")
        await call.message.answer("Проверка пачки завершена.")

    @dp.callback_query(F.data == "snipe_menu")
    async def snipe_menu(call: types.CallbackQuery, state: FSMContext):
        await call.message.answer("Введи юзернейм для мониторинга (без @):")
    ...
    async def add_snipe(m: types.Message, state: FSMContext):
        ...
        if m.from_user.id not in sniper.tasks:
            sniper.tasks[m.from_user.id] = asyncio.create_task(sniper._loop(m.from_user.id))
        await m.answer(f"📍 Начал следить за @{name}")
        await state.clear()

    @dp.callback_query(F.data == "check_one")
    async def check_one(call: types.CallbackQuery, state: FSMContext):
        await call.message.answer("Какой ник проверить?")
        await state.set_state(UsernameStates.waiting_for_quick_check)
    ...
    async def quick_check(m: types.Message, state: FSMContext):
        ...
        if await mgr.check_username_available(name):
            kb = InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="⚡️ ЗАБРАТЬ В ЧАТ", callback_data=f"claim_now_{name}")
            ]])
    ...
    print("🤖 Бот запущен. Авторизуйся в консоли!")
